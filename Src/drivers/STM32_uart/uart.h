...
enabled (RM0433 p.2046), the MCU buffers multiple bytes for UART TX and RX.
Each FIFO's size is 16 words (RM0433 p.2041).

We choose to use FIFO mode for a potential performance improvement by buffering multiple bytes when appropriate.
We are using DMA mode for UART, so the programmed thresholds for interrupts
should not have any effect on our operation since we do not use those interrupts.

From documentation (RM0433 p.2080-2082), it seems UART + DMA does not wait for any threshold
or multiple bytes before transferring, it simply transfers one byte whenever possible
(TX FIFO is not full or RX FIFO is not empty). Since our number of bytes to transfer
varies between transfers (and is not known in advance for RX), this suits our
application.

Note for DMA transfers: The buffer/array in memory MUST be accessible by DMA1
for the DMA transfer to work. For example, the STM32H743 has DTCM RAM which is
not accessible by DMA1/DMA2 (only by the MDMA). If you try to use DMA1 with a
buffer in DTCM, a DMA transfer error is generated. Sometimes, a project
...
mode, there is a programmed threshold (multiple bytes) for triggering transfers (RM0433 p.640-641,648-649).
For TX, it only transfers from memory to the FIFO when the FIFO is at or below the
threshold. For RX, it only transfers from the FIFO to memory when the FIFO is at
or above the threshold.

We use the DMA FIFO mode for TX, but not RX. For TX, we know exactly how many
bytes are to be transferred and program the DMA with that number, so the DMA transfers all of them, even if it is not
a multiple of the FIFO threshold. For RX, we use the buffer size to program the DMA with the
number of bytes to transfer (max possible number of RX bytes), but the actual number of
bytes transferred is less than that and is unknown in advance. As a result, even
with a 1/4 FIFO threshold, the number of RX bytes must be a multiple of 4 for the
DMA to transfer all of the bytes to the memory buffer. Because we want it to work
