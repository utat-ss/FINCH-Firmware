 ...
 */
uint8_t tim_init_function(TIM_HandleTypeDef* handle, uint16_t function, uint32_t onepulse_mode, TIM_Encoder_InitTypeDef* sConfig){
	if(function==TIM_Base){
		HAL_TIM_Base_MspInit(handle);
		HAL_TIM_Base_Init(handle);
	}else if(function==TIM_IC){
		HAL_TIM_IC_MspInit(handle);
		HAL_TIM_IC_Init(handle);
	}else if(function==TIM_OC){
		HAL_TIM_OC_MspInit(handle);
